...
export const kadoReadShape = {
	...
	limit: z.number().int().positive().optional().describe('Number of Unicode code points to return. Required for mode=firstXChars. Ignored for other modes.'),
	...
	headingPath: z.array(z.string()).optional().describe('Hierarchical heading path, e.g. ["Chapter 1", "Section A"], for precise disambiguation. Used with mode=section. Mutually exclusive with heading.'),
	...
	start: z.number().int().nonnegative().optional().describe('Start of the range. For rangeBasis=line: 1-based line number (inclusive). For rangeBasis=char: 0-based code-point offset (inclusive). Required for mode=range.'),
	end: z.number().int().nonnegative().optional().describe('End of the range. For rangeBasis=line: line number (inclusive). For rangeBasis=char: code-point offset (exclusive). Required for mode=range.'),
};
...
export const kadoWriteShape = {
	...
	headingPath: z.array(z.string()).optional().describe('Hierarchical heading path, e.g. ["Chapter 1", "Section A"], for precise disambiguation. Used with mode=insertUnderHeading or mode=replaceSection. Mutually exclusive with heading.'),
	...
	start: z.number().int().nonnegative().optional().describe('Start of the range to replace. For rangeBasis=line: 1-based line number (inclusive). For rangeBasis=char: 0-based code-point offset (inclusive). Required for mode=replaceRange.'),
	end: z.number().int().nonnegative().optional().describe('End of the range to replace. For rangeBasis=line: line number (inclusive). For rangeBasis=char: code-point offset (exclusive). Required for mode=replaceRange.'),
};
