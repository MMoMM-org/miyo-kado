...
<!-- 2026-06-20 -->
## Search/list shows phantom or missing files after restart / external bulk changes — Status: by-design (documented)
**Problem**: `kado-search` (and its `listDir`/`listNotes`) can return a path that no longer exists on disk, or miss a file that does exist, after Obsidian restarts or after files are changed **outside Obsidian's API** (git pull, rsync, another tool, edits while Obsidian was closed).
**Root cause**: every enumeration in `src/obsidian/search-adapter.ts` reads Obsidian's **in-memory index**, never the filesystem directly — `byName` over `vault.getFiles()`, `byTag`/`byContent`/`byFrontmatter`/`listTags`/`listNotes` over `vault.getMarkdownFiles()` + `metadataCache`, `listDir` over `folder.children`. The filesystem adapter (`vault.adapter.list()`/`exists()`) is used **only** by the audit logger in `main.ts`, not by search. That index is filled asynchronously by Obsidian's file watcher and startup re-scan, so it lags external changes; if the OS watcher drops events during a large batch it can stay stale until a manual reload.
**Two cases** — (A) changes Obsidian observed: detectable via `metadataCache.on('resolved')` / `vault.on('create'|'delete'|'rename')` — Kado currently subscribes to none of these (only `workspace.onLayoutReady` in `main.ts:166`). (B) changes Obsidian did NOT observe (dropped watcher events): **no signal exists** — Obsidian believes it is fresh; the only detection is an active `vault.adapter.list()` vs `vault.getFiles()` diff.
**Not the same as** the retracted "stale cache truncation" claim (see `docs/upstream-bugs/vault-cache-truncation.md`) — that was about write/read content and was disproven. This is purely about *enumeration* lag and never corrupts data: content I/O uses `vault.read`/`vault.process` (uncached), and heading writes fail safe with CONFLICT.
**Decision (2026-06-20)**: document only. A "fresh/stale" status-bar badge was considered and rejected — it would show false "fresh" in case B exactly when the problem occurs. (Side note: Kado has no status bar; the early `addStatusBarItem('Status bar text')` was Obsidian sample-plugin boilerplate, removed in scaffold commit `86c27d6`.)
**Workaround**: after a restart or external bulk change, let Obsidian finish indexing or reload/re-index the vault, then retry the search. A phantom hit also surfaces as `NOT_FOUND` on the follow-up `kado-read`/`kado-write`.
**Revisit if**: phantom/missing results become a recurring complaint → consider an opt-in `vault.adapter.exists()` guard in `byName`/`listDir`, or a manual "re-scan" affordance.
