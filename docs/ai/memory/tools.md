...
- **MCP-SDK Zod-Rejection Format:** Wenn ein Zod-Schema einen Tool-Call rejected, gibt das MCP SDK einen non-JSON Error-String an den Client zurück (statt eines strukturierten `{code, message}` JSON). Das bricht client-side `parseResult(JSON.parse)` mit `PARSE_ERROR`. Workaround: Zod-Schema breit halten (alle semantisch möglichen Werte akzeptieren), dann im Request-Mapper mit eigener VALIDATION_ERROR ablehnen. Beispiel: `kado-delete` operation-enum akzeptiert alle 4 DataTypes inkl. `dataview-inline-field`, der Mapper rejectet es aber mit sauberer JSON-Error-Response.
