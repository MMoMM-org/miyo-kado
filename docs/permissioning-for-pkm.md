...
> **Scope of this document.** This surveys the broader landscape of AI-vs-vault access control — where the real boundaries sit, how they tend to fail, and what serious permissioning actually looks like. MiYo Kado appears further down as one reference implementation, but not everything listed here is something Kado solves or can solve. Risks like prompt injection buried in your own notes, agents that rewrite their own configuration, sandbox prompts you click through without reading, or data pasted into third-party chatbots sit outside Kado's reach and need separate mitigations — note hygiene, tool choice, habits, and sometimes just awareness. The goal is to give you a realistic impression of what full vault access to an AI agent actually means for you, so you can decide which parts of the problem you want to enforce and which you are willing to live with.
...
- **"Dangerously disable sandbox" is a button the model itself presses.** Beyond the path prompt above, Claude Code's Bash tool exposes an explicit `dangerouslyDisableSandbox` parameter, and the model is instructed to retry with that flag set whenever a command appears to fail for sandbox reasons — a TLS error from a non-whitelisted host, a permission denied on a path, a unix socket error. Each retry is still a permission prompt, but the prompt arrives attached to an otherwise legitimate-looking workflow command (`git push`, `gh pr create`, `curl https://…`), so users in the flow of real work approve it to keep moving. The sandbox therefore defaults to "on" but yields routinely to any plausible-looking command, which is a soft default rather than a boundary. In the single session that produced this paragraph, Claude Code used this escape four times; every prompt was approved without scrutiny.
