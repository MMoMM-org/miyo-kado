...
- [x] **T1.1 Canonical types + default factories** `[activity: domain-modeling]`
...
- [x] **T1.2 Config migration (default-merge)** `[activity: backend-api]` `[parallel: true]`
...
- [x] **T1.3 Feature-gate pure function `gateOpenNoteScope`** `[activity: backend-api]` `[parallel: true]`
...
- [x] **T1.4 Workspace adapter `enumerateOpenNotes`** `[activity: backend-api]` `[parallel: true]`
...
- [x] **T1.5 Phase 1 Validation** `[activity: validate]`
  ...
  **Validation results (2026-04-20):**
  - `npx vitest run` → 852 tests pass across 40 files (+46 new)
  - `npm run lint` → clean
  - `npx tsc --noEmit` (src) → clean
  - `npx tsc --noEmit -p tsconfig.test.json` → only pre-existing debt in unrelated test files; T1.x test files clean
  - `npm run build` → production bundle compiles; copied to test vault
  - ADR-2 (feature gate as pure function, not in permission chain): `src/core/gates/open-notes-gate.ts` exports `gateOpenNoteScope` as a pure function; not added to `permission-chain.ts` ✓
  - ADR-3 (known view types only): `KNOWN_VIEW_TYPES = ['markdown', 'canvas', 'pdf', 'image']` in `open-notes-adapter.ts`; no `iterateAllLeaves` ✓
  - ADR-6 (per-key default OFF, AND with global, no inheritance): `activeOn = global.allowActiveNote && key.allowActiveNote` in gate; both flags default `false` in `createDefaultSecurityConfig` and via config-manager migration ✓
  - Commit hashes reference SDD: `ab6f149` (T1.1), `3a60d22` (T1.2), `c95ecd8` (T1.3), `03732a4` (T1.4)
