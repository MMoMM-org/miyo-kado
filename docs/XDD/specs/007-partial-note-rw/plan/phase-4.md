...
- [ ] **T4.1 `readNote` partial slicing** `[activity: backend-obsidian]`

  1. Prime: Read `src/obsidian/note-adapter.ts` (`readNote`, `stripFrontmatter`, metadataCache usage) and `[ref: SDD/Implementation Examples/Example 2]`
  2. Test: `test/obsidian/note-adapter.test.ts` (mocked App/metadataCache) — `firstXChars` returns slice + `truncated`; `section` by heading text returns content to next equal-or-higher heading (or EOF); `section` by `headingPath` disambiguates duplicate headings; missing section → `NOT_FOUND` naming it; `range` (line & char) returns span + `truncated`, over-EOF clamps; omitted `mode` → full body identical to today; result still carries `modified` for use as `expectedModified`
