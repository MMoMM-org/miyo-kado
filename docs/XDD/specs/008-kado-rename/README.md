...
| 2026-06-15 | Live-test finding: renameFile hangs when auto-update-links is off | `fileManager.renameFile` pops a blocking confirmation modal an MCP caller can't answer. Fix (ADR-8): conditional tool registration + opt-in setting (`renameWhenLinkUpdateOff`, behind a confirm modal) + configurable timeout (`renameTimeoutMs`, default 60 s). Kado never mutates the vault setting. |
| 2026-06-15 | Live-verified: the file moves immediately; dialog gates only link update | Refined ADR-8: on timeout the handler checks the vault and returns success with `linkUpdatePending: true` (rename already happened; links pending the user's dialog) — bare `TIMEOUT` only if the file did not move. Settings/modal/README/api-reference reworded: renames work; with auto-update off each rename prompts a link-update dialog. |
