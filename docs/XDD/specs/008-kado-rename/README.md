...
| 2026-06-15 | Live-test finding: renameFile hangs when auto-update-links is off | `fileManager.renameFile` pops a blocking confirmation modal an MCP caller can't answer. Fix (ADR-8): conditional tool registration + opt-in setting (`renameWhenLinkUpdateOff`, behind a confirm modal) + configurable timeout (`renameTimeoutMs`, default 60 s) returning a new `TIMEOUT` error. Kado never mutates the vault setting. |
