...
# Outer: rm with -r/-R flag (combined: -rf/-fr, or separate: -r -f / -f -r)
if echo "$COMMAND" | grep -qE 'rm\s+(-[a-zA-Z]*[rR][a-zA-Z]*|-[a-zA-Z]+\s+-[a-zA-Z]*[rR][a-zA-Z]*)'; then
    if echo "$COMMAND" | grep -qE '(\s/(\s|$)|\$HOME(\s|/|$)|~(\s|/|$)|\.claude(\s|/|$)|\.ssh(\s|/|$)|\.gnupg(\s|/|$))'; then
        block "rm -r on critical directory blocked (/, ~, .claude, .ssh, .gnupg)"
    fi
fi
